...
class MACDBacktester(): 
    ...
    def __repr__(self):
        return "MACDBacktester(symbol = {}, MACD({}, {}, {}), start = {}, end = {}, api = {})".format(self.symbol, self.EMA_S, self.EMA_L, self.signal_mw, self.start, self.end, self.api)
    ...
    def get_data(self):
        ...
        raw["EMA_S"] = raw['close'].ewm(span = self.EMA_S, min_periods = self.EMA_S).mean() 
        raw["EMA_L"] = raw['close'].ewm(span = self.EMA_L, min_periods = self.EMA_L).mean()
    ...
    def test_strategy(self):
        ...
        data["Cumulative Returns MACD Strategy"] = data["strategy"].cumsum().apply(np.exp)
        self.results = data
        
        perf = data["Cumulative Returns MACD Strategy"].iloc[-1] # absolute performance of the strategy
    ...
    def plot_results(self):
        ...
        if self.results is None:
            ...
            self.results[["Cumulative Returns Buy and Hold", "Cumulative Returns MACD Strategy"]].plot(title=title, figsize=(12, 8))
